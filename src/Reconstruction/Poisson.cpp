...
namespace reconstruction
{
    ...
    std::shared_ptr<geometry::TriangleMesh> NaivePoisson(const geometry::PointCloud &pcd, int max_depth)
    {
        ...
        // octree.UniformSplit();
        octree.BuildTreeUniformly(pcd);
        
        auto points = pcd.points;
        auto normals = pcd.normals;
        octree.SetNodeIDAndGetAllNodes();

        std::vector<geometry::OctreeNode *> &nodes = octree.all_nodes.back();
        ...
        for(size_t i = 0; i != d_points.size(); ++i)
        {
            ...
            for(int j = 0; j != 8; ++j)
            {
                ...
                auto n = octree.Locate(tmp_p, max_depth);
                // std::cout<<n<<std::endl;
                if(n != nullptr)
                {
        ...
        auto &final_nodes = octree.all_nodes.back();
        for(size_t i = 0; i != points.size(); ++i)
        {
            for(size_t k = 0; k != final_nodes.size(); ++k)
            {
                auto current_node = final_nodes[k];
        ...
        // cube_handler.SetVoxelResolution(0.01);
    ...
    std::shared_ptr<geometry::TriangleMesh> Poisson(const geometry::PointCloud &pcd, int max_depth)
    {
        ...
        for(int iter = 1; iter <= max_depth; ++iter)
        {
            ...
            for(size_t i = 0; i != d_points.size(); ++i)
            {
                ...
                for(int j = 0; j != 8; ++j)
                {
                    ...
                    auto located_node = octree.LocateAndSplit(tmp_p, iter);
                    if(located_node)
                    {
                        double width = nodes[i]->width;
                        for(int x = -1; x <= 1; ++x)
                        for(int y = -1; y <= 1; ++y)
                        for(int z = -1; z <= 1; ++z)
                        {
                            geometry::Point3 offset(x, y, z);
                            octree.LocateAndSplit(located_node->center + width * offset, iter);
                        }
        ...
        for(size_t i = 0; i != points.size(); ++i)
        {
            for(size_t j = all_nodes.size() - 1; j != all_nodes.size(); ++j)
            {
