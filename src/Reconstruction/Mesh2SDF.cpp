...
#pragma omp parallel for
    ...
    std::vector<double> Mesh2Indicator(const geometry::TriangleMesh &mesh, const geometry::Point3i & grid_num, geometry::Point3 origin, float voxel_resolution)
    {
        if(!mesh.HasNormals())
        {
    ...
    std::vector<double> NormalPCD2Indicator(const geometry::PointCloud &pcd, const geometry::Point3i & grid_num, geometry::Point3 origin, float voxel_resolution)
    {
        geometry::KDTree<3> kdtree;
...
#pragma omp parallel for
    ...
    geometry::PointCloud Centerline2SurfacePoints(const geometry::Point3List &centers, const std::vector<double> &radius, size_t n_points)
    {
        ...
        graph.ConstructEdgeAdaptive(2, 5);
    ...
    {
        geometry::Graph graph(centers);
        graph.ConstructEdgeAdaptive(1.5, 8);
...
#pragma omp parallel for
    ...
    void CenterLine2SDF(const geometry::Point3List &centers, const std::vector<double> &radius, CubeHandler &cube_handler, float voxel_resolution, bool ordered, int knn)
    {
