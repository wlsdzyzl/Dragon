...
namespace io
{
    ...
    void PlyDataToVec4List(const std::shared_ptr<PlyData> &p_data, geometry::Vec4List &point_list)
    {
        size_t count = p_data->count;
        //std::cout<<count<<std::endl;
        point_list.resize(count);
        if (p_data->t == tinyply::Type::FLOAT32)
        {
            ...
            for(size_t i = 0; i < count; ++i)
            {
                point_list[i] = geometry::Vector4((geometry::scalar)(*(data_ptr + i*4 + 0)), 
                    (geometry::scalar)(*(data_ptr + i*4 + 1)), (geometry::scalar)(*(data_ptr + i*4 + 2)), 
                    (geometry::scalar)(*(data_ptr + i*4 + 3)));
            }
        }
        else if (p_data->t == tinyply::Type::FLOAT64)
        {
    ...
    void PlyDataToPoint2uiList(const std::shared_ptr<PlyData> &p_data, geometry::Point2uiList &point_list)
    {
        if (p_data->t == tinyply::Type::UINT32 || p_data->t == tinyply::Type::INT32)
        {
            size_t count = p_data->count;
            ...
            for(size_t i = 0; i < count; ++i)
            {
                point_list[i] = geometry::Point2ui((unsigned int)(*(data_ptr + i*2 + 0)), 
                    (unsigned int)(*(data_ptr + i*2 + 1)));
            }
        }
    ...
    // read ply with edges
    bool ReadPLY(const std::string &filename, geometry::Point3List &_points, 
        geometry::Point3List &_normals,
        geometry::Point3List &_colors, geometry::Point2uiList &_edges,
        std::vector<AdditionalElement> & additional_labels)
    {
        std::unique_ptr<std::istream> file_stream;
        std::vector<uint8_t> byte_buffer;
        ...
        try
        {
            ...
            // Because most people have their own mesh types, tinyply treats parsed data as structured/typed byte buffers. 
            // See examples below on how to marry your own application-specific data structures with this one. 
            std::shared_ptr<PlyData> vertices, normals, colors, edges;
            std::vector<std::shared_ptr<PlyData>> additional_elements_ptr(additional_labels.size());
            // The header information can be used to programmatically extract properties on elements
            // known to exist in the header prior to reading the data. For brevity of this sample, properties 
            // like vertex position are hard-coded: 
...

            {
                ...
                PlyDataToPoint3List(colors, _colors);
                if(edges)
                PlyDataToPoint2uiList(edges, _edges);
                for(size_t i = 0; i != additional_labels.size(); ++i)
                {
                    if(additional_elements_ptr[i])
                    {
    ...
    // read ply with radius and edges
        ...
        std::vector<AdditionalElement> & additional_labels)
    {
        std::unique_ptr<std::istream> file_stream;
        std::vector<uint8_t> byte_buffer;
        geometry::Vec4List _points_with_radius;
        ...
        try
        {
            ...
            // known to exist in the header prior to reading the data. For brevity of this sample, properties 
            // like vertex position are hard-coded: 
            try { vertices = file.request_properties_from_element("vertex", { "x", "y", "z", "r" }); }
            catch (const std::exception & e) { std::cout << RED << "[ERROR]::[ReadPLY]::" << e.what() << RESET<< std::endl; return false;}
...

            {
                //processing points, normals, colors, triangles
                if(vertices)
                PlyDataToVec4List(vertices, _points_with_radius);
                if(normals)
                PlyDataToPoint3List(normals, _normals);
                if(colors)
                PlyDataToPoint3List(colors, _colors);
                ...
                _points.resize(_points_with_radius.size());
                _radius.resize(_points_with_radius.size());
                for(size_t i = 0; i != _points_with_radius.size(); ++i)
                {
                    _points[i](0) = _points_with_radius[i](0);
                    _points[i](1) = _points_with_radius[i](1);
                    _points[i](2) = _points_with_radius[i](2);
                    _radius[i] = _points_with_radius[i](3);
                }
            }
        }
        catch (const std::exception & e)
        ...
        bool use_ascii)
    {
        ...
        for(size_t i = 0; i != neighbors.size(); ++i)
        {
            ...
            for(size_t j = 0; j != neighbors[i].size(); ++j )
                if(neighbors[i][j] > i) edges.push_back(std::make_pair(i, neighbors[i][j]));
        }
        ...
        {
            edges_buffer.push_back((unsigned int)edges[i].first);
            edges_buffer.push_back((unsigned int)edges[i].second);
        }        
        ...
        geometry_file.add_properties_to_element("edge", { "vertex1", "vertex2" },
            Type::UINT32, edges_buffer.size() / 2, reinterpret_cast<uint8_t*>(edges_buffer.data()), Type::INVALID, 0);
    ...
    {
        // ignore radius
        assert(radius.size() != points.size()&&"radius and points should have the same length.");
        geometry::Vec4List xyzr(points.size());
