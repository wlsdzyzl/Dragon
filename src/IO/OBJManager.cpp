...
namespace io
{
        ...
        geometry::Point3List &colors)
    {
        ...
        bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err,
                                    filename.c_str());
        if (!warn.empty()) {
            std::cout<<YELLOW<<"[WARNNING]::[LoadFromOBJ]::"<<warn<<" (Notice that currently we don't support texture, uv coordinate in OnePiece, because I know little about it)"<<RESET<<std::endl;
        }
        if (!err.empty()) {
