...
namespace geometry
{
    ...
    void Graph::ConstructEdgeRadius(const geometry::ScalarList & radius)
    {
    ...
    bool Graph::LoadFromPLY(const std::string &filename)
    {
        // through edges
        geometry::Point2uiList _edges;
        std::vector<io::AdditionalElement> additional_labels;
        bool res = io::ReadPLY(filename, vertices, colors, normals, _edges, additional_labels);
        edges.resize(_edges.size());
        for(size_t i = 0; i != _edges.size(); ++i)
        {
            edges[i].start = _edges[i][0];
            edges[i].end = _edges[i][1];
        }
        ComputeNeighbors();
        return res;
    }
    ...
    bool SkeletonGraph::LoadFromPLY(const std::string &filename)
    {
        // through edges
        geometry::Point2uiList _edges;
        std::vector<io::AdditionalElement> additional_labels;
        // std::cout<<"Load from PLY"<<std::endl;
        bool res = io::ReadPLY(filename, vertices, radius, colors, normals, _edges, additional_labels);
        edges.resize(_edges.size());
        for(size_t i = 0; i != _edges.size(); ++i)
        {
            edges[i].start = _edges[i](0);
        ...
        ComputeNeighbors();
        assert(radius.size() == vertices.size());
        return res;
    }
    bool SkeletonGraph::WriteToPLY(const std::string &filename) const
    {
        // through neighbors
        return io::WritePLY(filename, vertices, radius, colors, normals, neighbors);
    }
}
}
